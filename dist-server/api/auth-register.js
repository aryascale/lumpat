...
export default async function handler(req) {
    ...
    try {
        ...
        const existingUsers = await query('SELECT id, email, username FROM User WHERE email = ? OR username = ? LIMIT 1', [email, username]);
        ...
        await query('INSERT INTO User (id, email, username, password, role, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, NOW(), NOW())', [userId, email, username, hashedPassword, 'user']);
