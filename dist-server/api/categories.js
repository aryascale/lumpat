...
import { getDefaultCategories, saveDefaultCategories, resetDefaultCategories } from '../src/lib/defaultCategories.js';
const CORS_HEADERS = {
...
export default async function handler(event) {
    ...
    try {
        ...
        if (event.httpMethod === 'GET') {
            ...
            return { statusCode: 200, headers: CORS_HEADERS, body: JSON.stringify({ categories: categories.map((c) => c.name) }) };
        }
        if (event.httpMethod === 'POST' || event.httpMethod === 'PUT') {
            ...
            if (!Array.isArray(categories))
                return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ error: 'categories must be an array' }) };
            if (categories.length === 0)
                return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ error: 'At least one category is required' }) };
            if (isDefault) {
            ...
            await prisma.category.createMany({
                data: categories.map((name, order) => ({ eventId, name, order })),
            });
            const updated = await prisma.category.findMany({
                where: { eventId },
                orderBy: { order: 'asc' },
            });
            return { statusCode: 200, headers: CORS_HEADERS, body: JSON.stringify({ categories: updated.map((c) => c.name) }) };
        }
