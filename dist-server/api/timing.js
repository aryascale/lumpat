...
export default async function handler(event) {
    ...
    try {
        ...
        if (event.httpMethod === 'GET') {
            const events = await query('SELECT id, cutoffMs, categoryStartTimes FROM Event WHERE id = ? LIMIT 1', [eventId]);
            if (events.length === 0)
            ...
            if (typeof categoryStartTimes === 'string') {
                try {
                    categoryStartTimes = JSON.parse(categoryStartTimes);
                }
                catch { }
            }
        ...
        if (event.httpMethod === 'POST' || event.httpMethod === 'PUT') {
            ...
            const existing = await query('SELECT id FROM Event WHERE id = ? LIMIT 1', [eventId]);
            if (existing.length === 0)
                return errorResponse('Event not found', 404);
            await query('UPDATE Event SET cutoffMs = ?, categoryStartTimes = ?, updatedAt = NOW() WHERE id = ?', [cutoffMs ?? null, categoryStartTimes ? JSON.stringify(categoryStartTimes) : null, eventId]);
            const updated = await query('SELECT id, cutoffMs, categoryStartTimes FROM Event WHERE id = ? LIMIT 1', [eventId]);
            let parsedCST = updated[0].categoryStartTimes;
            if (typeof parsedCST === 'string') {
                try {
                    parsedCST = JSON.parse(parsedCST);
                }
                catch { }
            }
