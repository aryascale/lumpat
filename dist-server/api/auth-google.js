...
const client = new OAuth2Client(process.env.VITE_GOOGLE_CLIENT_ID, process.env.GOOGLE_CLIENT_SECRET, 'postmessage');
export default async function handler(req) {
    ...
    try {
        ...
        if (!payload || !payload.email)
            return errorResponse('Invalid Google token payload', 400);
        const { email, sub: googleId, name } = payload;
        let users = await query('SELECT * FROM User WHERE email = ? LIMIT 1', [email]);
        let user = users[0];
        if (!user) {
            const baseUsername = email.split('@')[0];
            let username = baseUsername;
            let counter = 1;
            let existing = await query('SELECT id FROM User WHERE username = ? LIMIT 1', [username]);
            while (existing.length > 0) {
                username = `${baseUsername}${counter++}`;
                existing = await query('SELECT id FROM User WHERE username = ? LIMIT 1', [username]);
            }
            const userId = crypto.randomUUID();
            await query('INSERT INTO User (id, email, username, googleId, name, isPhoneVerified, role, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())', [userId, email, username, googleId, name, false, 'user']);
