...
export default async function handler(event) {
    ...
    try {
        ...
        if (!boundary)
            return errorResponse('Missing boundary', 400);
        if (!event.body)
        ...
        await query('INSERT INTO Banner (id, eventId, imageUrl, alt, `order`, isActive, createdAt) VALUES (?, ?, ?, ?, ?, ?, NOW())', [bannerId, eventId, result.url, fields.alt || file.name, parseInt(fields.order || '0', 10), true]);
