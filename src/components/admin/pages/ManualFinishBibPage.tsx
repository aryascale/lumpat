...
interface ManualFinishBibPageProps {
  allRows: LeaderRow[];
  onDataVersionBump: () => void;
  eventId: string;
}

interface ManualFinishRecord {
  id: string;
  bib: string;
  epc: string;
...
export default function ManualFinishBibPage({ allRows, onDataVersionBump, eventId }: ManualFinishBibPageProps) {
  const [q, setQ] = useState("");
  const [manualFinishes, setManualFinishes] = useState<ManualFinishRecord[]>([]);
  ...
  const loadManualFinishes = useCallback(async () => {
    try {
      const res = await fetch(`/api/manual-finish-bib?eventId=${eventId}`);
      if (res.ok) {
        const data = await res.json();
        setManualFinishes(Array.isArray(data) ? data : []);
      }
    } catch (error) {
      console.error("Failed to load manual finishes:", error);
    } finally {
      setLoading(false);
    }
  }, [eventId]);
  ...
  useEffect(() => {
    loadMasterData();
    loadManualFinishes();
  }, [loadMasterData, loadManualFinishes]);
  ...
  const applyManualFinish = async (row: MasterRow) => {
    if (!timeStr) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || "Gagal menyimpan Manual Finish per-BIB");
      }

      await loadManualFinishes();
      onDataVersionBump();
      setSelectedBib(null);
    } catch (error: any) {
      alert(error.message);
  ...
  const removeManualFinish = async (id: string) => {
    if (!confirm("Hapus Manual Finish ini? Waktu finish pelari akan kembali pakai CSV/Decoder.")) return;

    try {
      const res = await fetch(`/api/manual-finish-bib?eventId=${eventId}&id=${id}`, {
        method: "DELETE",
      });

      if (!res.ok) throw new Error("Gagal menghapus manual finish");
    ...
    <div className="flex flex-col mt-6">
      <div className="card flex-1 flex flex-col mb-4">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
          <div>
            <h2 className="section-title">Individual Manual Finish (Per BIB)</h2>
            <div className="subtle text-sm">
              Override waktu finish khusus untuk masing-masing BIB. Format HH:MM:SS.
            </div>
          </div>
          <input
            ...
            <table className="f1-table compact">
              <thead>
                <tr>
                  ...
                  <th style={{ width: 140 }}>FINISH TIME (CSV/DEC)</th>
                  <th style={{ width: 140 }}>MANUAL FINISH</th>
                  ...
                  currentRows.map((r) => {
                    ...
                    return (
                      <tr key={r.epc} className="row-hover">
                        ...
                        <td className="mono text-xs text-rose-600 font-bold">
                          {msRecord ? (
                            msRecord.timeStr
                          ) : (
                            "-"
                              ...
                              {msRecord ? (
                                <>
                                  ...
                                  <button
                                    ...
                                    onClick={() => removeManualFinish(msRecord.id)}
                                  >
                                    Remove
                                  </button>
                                </>
                                ...
                                <button
                                  className="btn ghost sm text-rose-600 border border-rose-200"
                                  disabled={saving}
                ...
                <div key={r.epc} className="bg-white border border-gray-200 rounded-lg p-3 shadow-sm">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <div className="text-xs text-gray-400 mt-0.5 mono">Finish (CSV/Dec): {r.finishTimeRaw || "-"}</div>
                    </div>
                    <span className="mono text-xs mt-0.5 font-bold text-rose-600">
                      {msRecord ? msRecord.timeStr : "-"}
                    </span>
                  </div>
                  ...
                  {isEditing ? (
                    <div className="space-y-2 mt-2 pt-2 border-t border-gray-100">
                      ...
                      <div className="flex gap-2">
                        <button
                          className="btn flex-1 text-xs font-bold bg-rose-600 hover:bg-rose-700"
                          disabled={saving}
                          onClick={() => applyManualFinish(r)}
                        >
                          {saving ? "Saving..." : "Apply Finish Time"}
                        </button>
                        <button
                    ...
                    <div className="flex gap-2 mt-2">
                      {msRecord ? (
                        ...
                        <button
                          className="btn ghost w-full text-xs font-bold uppercase text-rose-600 border border-rose-200"
                          disabled={saving}
                          onClick={() => assignCurrentBrowserTime(r)}
                        >
                          + Assign Finish Time
