...
export function SlideToConfirm({
  ...
  const trackWidth = Math.max(containerWidth - height, 0); // Total drag distance
  ...
  const bgWidth = useTransform(x, [0, trackWidth], [height, containerWidth]);
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "relative flex items-center justify-center overflow-hidden rounded-full border bg-stone-100 select-none",
    ...
    >
      ...
      <motion.div
        ...
        style={{
          width: state === "success" ? containerWidth : bgWidth,
          backgroundColor: state === "success" ? "#22c55e" : "#e5e5e5",
          opacity: state === "success" ? 0.1 : 0.5,
        }}
        animate={{ width: state === "success" ? containerWidth : undefined }}
      ...
      <motion.span
        className={cn(
          "absolute font-bold text-xs uppercase tracking-widest z-0",
          state === "success" ? "text-green-600 dark:text-green-400" : "text-stone-500"
        )}
      ...
      {/* Success Text */}
      <motion.span
        className="absolute font-bold text-xs uppercase tracking-widest z-0 text-green-600 dark:text-green-400"
      ...
      <motion.div
        ...
        className={cn(
          "absolute left-1 z-10 flex cursor-grab items-center justify-center rounded-full bg-white shadow-md active:cursor-grabbing",
