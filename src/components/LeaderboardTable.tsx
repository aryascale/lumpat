...
export default function LeaderboardTable({
  ...
  const rankedRows = useMemo(() => {
    ...
    if (genderFilter !== "All") {
      ...
      currentRows = currentRows.filter((r) => {
        ...
        if (gFilter === "laki-laki")
          return g === "laki-laki" || g === "m" || g === "male" || g === "pria";
        if (gFilter === "perempuan")
          return (
            g === "perempuan" || g === "f" || g === "female" || g === "wanita"
          );
        return g === gFilter;
      });
    }
    if (ageCategoryFilter !== "All") {
      currentRows = currentRows.filter(
        (r) => r.ageCategory?.trim() === ageCategoryFilter,
      );
    }
  ...
  const podiums = useMemo(() => {
    ...
    if (ageCategoryFilter !== "All") {
      filteredForPodium = filteredForPodium.filter(
        (r) => r.ageCategory?.trim() === ageCategoryFilter,
      );
    }
  ...
  const getRowBorder = (rank: number | null) => {
    if (rank === 1)
      return "border-2 border-[#d4af37] shadow-[0_4px_15px_rgba(212,175,55,0.15)]";
    if (rank === 2)
      return "border-2 border-slate-300 shadow-[0_4px_15px_rgba(148,163,184,0.15)]";
    if (rank === 3)
      return "border-2 border-[#cd7f32] shadow-[0_4px_15px_rgba(205,127,50,0.15)]";
      ...
      {!hidePodium && podiums.length > 0 && (
        ...
        >
            ...
            {isPodiumFullscreen ? (
              ...
              >
                <path
                  ...
                  strokeWidth={2.5}
                  d="M9 9V4.5M9 9H4.5M15 9V4.5M15 9h4.5M9 15v4.5M9 15H4.5M15 15v4.5M15 15h4.5"
                />
              </svg>
            ) : (
              <svg
          ...
          {podiums.map((podium, pIdx) => (
            <div
              key={podium.title}
              className={`w-full relative z-10 flex flex-col items-center ${pIdx > 0 ? "mt-16 sm:mt-24 pt-12 sm:pt-16 border-t-[3px] border-dashed border-stone-200" : ""}`}
            >
              <motion.div
              ...
              >
                <h3
                  className={`font-black tracking-[0.2em] text-red-600 uppercase mb-2 ${isPodiumFullscreen ? "text-lg md:text-2xl" : "text-sm"}`}
                >
                  Podium
                </h3>
                <h2
                  className={`font-extrabold text-stone-900 tracking-tighter ${isPodiumFullscreen ? "text-5xl md:text-7xl" : "text-3xl sm:text-5xl"}`}
                >
                  {podium.title}
                </h2>
                <div
                  className={`font-bold text-stone-500 tracking-wide mt-3 ${isPodiumFullscreen ? "text-2xl md:text-3xl mb-8" : "text-base sm:text-xl"}`}
                >
                  {ageCategoryFilter === "All" ? "All Ages" : ageCategoryFilter}{" "}
                  {title.toLowerCase().includes("overall")
                    ? "Overall"
                    : title.replace("Full Standings — ", "").trim()}{" "}
                ...
                {podium.top3[1] && (
                  ...
                  >
                    <div className="flex flex-col items-center mb-2 sm:mb-4 w-full px-1 sm:px-2">
                      ...
                      <div
                        className={`font-extrabold text-stone-800 text-center line-clamp-2 w-full mt-2 leading-tight ${isPodiumFullscreen ? "text-lg md:text-2xl mt-4" : "text-[10px] sm:text-base"}`}
                      >
                        {podium.top3[1].name}
                      </div>
                      <div
                      ...
                      <div
                        className={`bg-slate-200 border-2 border-slate-300 border-b-4 text-stone-900 font-mono font-black rounded-xl shadow-sm ${isPodiumFullscreen ? "text-base md:text-xl px-4 py-2 mt-2" : "text-[10px] sm:text-sm px-2 py-0.5 sm:px-4 sm:py-1.5"}`}
                      >
                    ...
                    >
                      <span
                        className={`font-black text-black/10 drop-shadow-sm ${isPodiumFullscreen ? "text-8xl md:text-9xl" : "text-5xl sm:text-7xl"}`}
                      >
                        2
                      </span>
                    </div>
                ...
                {podium.top3[0] && (
                  ...
                  >
                    <div className="flex flex-col items-center mb-2 sm:mb-4 w-full px-1 sm:px-2 relative">
                      ...
                      <div
                        className={`font-extrabold text-stone-900 text-center line-clamp-2 w-full mt-2 leading-tight ${isPodiumFullscreen ? "text-xl md:text-3xl mt-4" : "text-[11px] sm:text-lg"}`}
                      >
                        {podium.top3[0].name}
                      </div>
                      <div
                      ...
                      <div
                        className={`bg-yellow-200 border-2 border-yellow-400 border-b-4 text-stone-900 font-mono font-black rounded-xl shadow-sm ${isPodiumFullscreen ? "text-lg md:text-3xl px-6 py-2 mt-2" : "text-[11px] sm:text-base px-3 py-1 sm:px-5 sm:py-1.5"}`}
                      >
                    ...
                    >
                      ...
                      <style>{`
                               ...
                               `}</style>
                      <span
                        className={`font-black text-black/10 drop-shadow-sm relative z-10 ${isPodiumFullscreen ? "text-9xl md:text-[12rem] leading-none" : "text-6xl sm:text-8xl"}`}
                      >
                        1
                      </span>
                    </div>
                ...
                {podium.top3[2] && (
                  ...
                  >
                    <div className="flex flex-col items-center mb-2 sm:mb-4 w-full px-1 sm:px-2">
                      ...
                      <div
                        className={`bg-orange-200 border-2 border-orange-300 border-b-4 text-stone-900 font-mono font-black rounded-xl shadow-sm ${isPodiumFullscreen ? "text-base md:text-xl px-4 py-2 mt-2" : "text-[10px] sm:text-sm px-2 py-0.5 sm:px-4 sm:py-1.5"}`}
                      >
                    ...
                    >
                      <span
                        className={`font-black text-black/10 drop-shadow-sm ${isPodiumFullscreen ? "text-7xl md:text-8xl" : "text-5xl sm:text-7xl"}`}
                      >
                        3
                      </span>
                    </div>
        ...
        <>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center sm:items-end gap-4 pb-5 mb-5 border-b border-slate-200">
            ...
            <div className="flex flex-wrap items-center gap-2.5 w-full sm:w-auto">
              ...
              <button
                className="px-3.5 py-2 font-semibold text-slate-500 bg-slate-100 hover:bg-slate-200 rounded transition-colors text-sm"
                onClick={() => {
                  setQ("");
                  setGenderFilter("All");
              ...
              <button
                ...
                className="px-4 py-2 font-semibold text-white bg-red-500 hover:bg-red-600 active:bg-red-700 rounded shadow-sm hover:shadow transition-all text-sm"
              >
                Export CSV
              </button>
            </div>
            ...
            <div className="w-full md:min-w-[800px] flex flex-col gap-2.5">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="hidden md:grid grid-cols-[44px_1fr_90px_90px_90px_110px_80px_28px] gap-2 items-center px-4 py-3">
                      ...
                      {/* Finish */}
                      <div className="font-mono text-[11px] text-rose-400 font-medium">
                        {r.finishTimeRaw || "-"}
                      </div>
                      ...
                      return (
                        <div className="flex gap-2 px-3 pb-2.5 overflow-x-auto">
                          {visibleLaps.map((lap, i) => (
                            <div
                              key={i}
                              className="flex-shrink-0 bg-slate-50 border border-slate-100 rounded px-2 py-1"
                            >
                              <div className="flex justify-between items-start gap-3 mb-0.5">
                                ...
                                <span
                                  className="text-[7px] font-bold text-slate-300 bg-slate-100/50 px-1 rounded-sm leading-none pt-[1px]"
                                  title={
                                    lap.isDuration
                                      ? "Duration from Start"
                                      : "Time of Day"
              ...
              {filtered.length === 0 && (
                <div className="text-center py-16 bg-slate-50 border border-dashed border-slate-200 px-4">
                  <div className="font-bold text-xl text-slate-300 mb-1 tracking-tight">
                    No Tracking Data
                  </div>
